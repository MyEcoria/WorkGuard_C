...
int open_lmdb(db_t *database, const char *db_path) {
    ...
    if (database->rc != 0) {
        fprintf(stderr, "Erreur lors de la création de l'environnement : %s\n", mdb_strerror(database->rc));
        return -1;
    }
    database->rc = mdb_env_set_maxdbs(database->env, 1);
    if (database->rc != 0) {
        fprintf(stderr, "Erreur lors de la configuration de l'environnement : %s\n", mdb_strerror(database->rc));
        return -1;
    }
    database->rc = mdb_env_open(database->env, db_path, 0, 0664);
    if (database->rc != 0) {
        fprintf(stderr, "Erreur lors de l'ouverture de l'environnement : %s\n", mdb_strerror(database->rc));
        return -1;
    }
    return 0;
}
...
int begin_transaction(db_t *database) {
    ...
    if (database->rc != 0) {
        fprintf(stderr, "Erreur lors de la création de la transaction : %s\n", mdb_strerror(database->rc));
        return -1;
    }
    return 0;
}
...
int write_in_db(db_t *database, const char *key, const char *value) {
    ...
    database->rc = mdb_put(database->txn, database->dbi, &database->key, &database->value, 0);
    if (database->rc != 0) {
        fprintf(stderr, "Erreur lors de l'écriture dans la base de données : %s\n", mdb_strerror(database->rc));
        return -1;
    }
...
char *get_value(db_t *database, const char *key) {
    ...
    database->rc = mdb_txn_begin(database->env, NULL, MDB_RDONLY, &database->txn);
    if (database->rc != 0) {
        fprintf(stderr, "Erreur lors de la transaction en lecture : %s\n", mdb_strerror(database->rc));
        return NULL;
    }
    ...
    char *result = strdup((char *)value2.mv_data);
